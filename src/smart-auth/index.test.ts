...
describe("an authorize url", () => {
  ...
  test("accepts scope overrides but falls back to defaults", async () => {
    ...
    expect(overrideResponse.headers['location']).toBe("http://external.localhost/oauth/authorize?response_type=code&client_id=123&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fsmart%2Fidp%2Fredirect&scope=launch%20patient%2FObservation.read&state=smart-auth-static-bytes-not-random-mock")
    ...
    expect(defaultResponse.headers['location']).toBe("http://external.localhost/oauth/authorize?response_type=code&client_id=123&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fsmart%2Fidp%2Fredirect&scope=launch&state=smart-auth-static-bytes-not-random-mock")
  })
})
