<<<<<<< HEAD
import { Service, Card, NoDecisionResponse, Hooks } from "../../src/cds-hooks";
...
const handler =  async (request: CDSHooks.HookRequest<any>) => {                            
  ...
  const medicationRequest = draftOrders.entry
    ?.find((e) => e.resource?.resourceType == "MedicationRequest") // All FHIR Resources have a resourceType field
    ?.resource as fhir4.MedicationRequest                          // https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#type-assertions
...
=======
import { Service, Card, NoDecisionResponse } from "../../src/cds-hooks";
import { comparePrice } from "./api";
...
export default new Service(
   ...
   */
  {
    id: "good-rx-comparison", // Used to define the HTTP routes for the Service
    title: "GoodRx Compare Price Service for MedicationRequest", // A narrative title used to generate UI config for Services by places like the LogicaSandbox
    hook: "order-select", // Identifies the "hook" context this service should be executed in (the hook determines what information will be sent)
    description: "GoodRx's Compare Price API is used to provide drug cost estimates during the prescription order workflow"
  },
  /**
   * ServiceHandler - the function that fires on the HookRequest
   */
  async (request: CDSHooks.HookRequest<any>) => {                            
     ...
     * into a small utility library)
     */
    const response = await comparePrice({ name: medicationRequest.medicationCodeableConcept?.text });
